...
local uiCallMethod_ = uiCallMethod
...
function DxNotificationProvider:enqueueNotification(obj)
    local notification = { uid = obj.uid, type = obj.type }
    ...
    setmetatable(notification, uiClass.mt)
...
local function renderNotifications()
    ...
    for i = len, 1, -1 do
        ...
        if (item._stackPos and i ~= item._stackPos or item._oldSize ~= len) then
            ...
            uiCallMethod_(item, "moveTo", moveX, currentY, ANIMATION_DURATION, "OutQuad")
        end
        ...
        if (not item._isHandled) then
            ...
            uiCallMethod_(item, "moveTo", SCREEN_WIDTH - item.width - NOTIFICATIONS_MARGIN, currentY, ANIMATION_DURATION, "OutQuad")
        end
        ...
        if ((item._duration <= 0 or item._requestClose) and not item._isClosing or (not item._open and not item._isClosing)) then
            ...
            uiCallMethod_(item, "moveTo", SCREEN_WIDTH + NOTIFICATIONS_MARGIN, currentY, ANIMATION_DURATION, "OutQuad")
        ...
        --item:customRenderer()
        uiCallMethod_(item, "customRenderer")
        item._stackPos = i
        item._oldSize = len
