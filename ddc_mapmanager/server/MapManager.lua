...
function MapManager:loadMapList()
	...
	for _, resource in ipairs(resourceList) do
		if (resource:getState() == "loaded" and resource:getInfo("type") == "map") then
			resourceName = resource:getName()
			mapName = resource:getInfo("name") or "Unknown"
			mapPrefix = mapName:match("%[(%w+)%]")
			mapAuthor = resource:getInfo("author") or "Unknown"
	...
	for _, prefix in pairs(self.mapTypes) do
