...
function SyncManager:setData(element, field, value, shouldSync)
	...
	if (shouldSync) then
		if (not self.storedElementData[element]) then
			...
			addEventHandler("onElementDestroy", element, self._onElementDestroy)
		end
...
function SyncManager:onServerReceiveClientElementSync(element, field, value)
	...
	if (element:getType() == "player" and not table.find(self.allowedClientSyncFields.player, field)) then
...
function SyncManager:sendInitialSyncToPlayer(player)
	...
	player:triggerEvent("onClientReceiveInitialElementSync", resourceRoot, syncTable)
end
