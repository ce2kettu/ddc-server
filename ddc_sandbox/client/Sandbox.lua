...
function Sandbox:toggleSounds(state)
    for _, sound in ipairs(getElementsByType("sound", self:getMapElement())) do
        if (state) then
            for _, mutedSound in ipairs(self.mutedSounds) do
                local soundElement = mutedSound.element

                if (isElement(soundElement)) then
                    setSoundVolume(soundElement, mutedSound.volume)
                else
                    table.remove(self.mutedSounds, i)
                end
            end
        else
            self.mutedSounds[#self.mutedSounds + 1] = { element = sound, volume = getSoundVolume(sound) }
